        ...
        <div class="row g-5">
            <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.1s">
                <div class="position-relative overflow-hidden ps-5 pt-5 h-100" style="min-height: 400px;">
                    <img class="position-absolute w-100 h-100" src="{{ asset('img/backwall3.png') }}" alt="" style="object-fit: cover; border-radius: 20px;">
                    <img class="position-absolute top-0 start-0 bg-white pe-3 pb-3 rounded-circle" src="{{ asset('img/148670012.png') }}" alt="" style="width: 200px; height: 200px;">
                </div>
            </div>
            ...
            <div class="col-lg-10 wow fadeInUp" data-wow-delay="0.2s">
                <div class="card border-0 shadow-lg rounded-4 text-center h-100" style="transition: all 0.4s; transform: translateY(0);" onmouseover="this.style.transform='translateY(-10px)'; this.style.boxShadow='0 25px 50px rgba(0,0,0,0.15)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 10px 40px rgba(0,0,0,0.1)';">
                    <div class="card-body p-5">
                        <div class="mb-3">
                            <img src="{{ asset('img/logomasehi.png') }}" width="60" height="60" alt="Logo">
                        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="{{ 0.1 + ($index * 0.1) }}s">
                <div class="card border-0 shadow h-100 rounded-4 overflow-hidden" style="transition: all 0.4s;" onmouseover="this.style.transform='translateY(-10px)'; this.style.boxShadow='0 20px 40px rgba(0,0,0,0.15)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 5px 20px rgba(0,0,0,0.1)';">
                    @if($item->gambar)
                    <img src="{{ asset('img/fasilitas/' . $item->gambar) }}" class="card-img-top" alt="{{ $item->nama }}" style="height: 200px; object-fit: cover;">
                    @else
                    <div class="bg-primary d-flex align-items-center justify-content-center" style="height: 200px;">
                        <i class="{{ $item->icon ?? 'fa fa-building' }} text-white fa-4x"></i>
                    </div>
                    @endif
                    <div class="card-body p-4">
                        <h5 class="mb-3">{{ $item->nama }}</h5>
                        <p class="text-secondary mb-0">{{ Str::limit($item->deskripsi, 100) }}</p>
                    </div>
                </div>
            </div>
            @endforeach
...
<div class="container-xxl py-5 bg-light">
    <div class="container">
        ...
        <div class="row g-4">
            @foreach($gurus->take(8) as $index => $guru)
            <div class="col-lg-3 col-md-6 wow fadeInUp" data-wow-delay="{{ 0.1 + ($index * 0.1) }}s">
                <div class="team-item position-relative rounded-4 overflow-hidden" style="transition: all 0.4s;" onmouseover="this.style.transform='translateY(-10px)';" onmouseout="this.style.transform='translateY(0)';">
                    <div class="position-relative">
                        <img class="img-fluid w-100" src="{{ asset('img/guru/' . $guru->foto) }}" alt="{{ $guru->nama }}" style="height: 280px; object-fit: cover;">
                    ...
                    <div class="bg-white text-center p-4">
                        ...
                        <span class="text-primary">{{ $guru->jabatan }}</span>
                    </div>
                </div>
            </div>
            @endforeach
